...
with tf.Graph().as_default() as graph:
    with tf.compat.v1.Session() as sess:
        ...
        with tf.io.gfile.GFile(model_dir, "rb") as f:
            ...
            logger.info("Time Taken for model inference in seconds ---> %f", time.time()-start_time)
            LAST_INDEX = 175
            CORRECT_PREDICTION_COUNTER = 0
            WRONG_PREDICTION_COUNTER = 0
            # Calculating prediction
            for step in range(175, 375, code_batch_size):
                x_batch, y_batch = read_Image(code_batch_size, LAST_INDEX, path_val_list)
                LAST_INDEX += code_batch_size
                start_time = time.time()
                correct_prediction_temp = sess.run(l_output, feed_dict={l_input: x_batch, l_output: y_batch})
                logger.info("Time Taken for model inference in seconds ---> %f", time.time()-start_time)
            ...
            print("the number of correct predcitions (TP + TN) is:", CORRECT_PREDICTION_COUNTER)
            print("The number of wrong predictions (FP + FN) is", WRONG_PREDICTION_COUNTER)
